...
class Goods extends BaseModel
{
  ...
  */
    ...
    {
        if($type=='new'){
            $data=self::with('imgs')->where('is_new',1)->select();
        }else if($type=='hot'){
            $data=self::with('imgs')->where('is_hot',1)->select();
        }else if($type=='recommend'){
            $data=self::with('imgs')->where('is_recommend',1)->select();
        }else{
            $data=self::with('imgs')->select();
        }
        if(!$data || count($data)<1){
    ...
    public static function getCatePros($cid)
    {
        ...
        $cwhere['pid']=$cid;
        $cid_arr=Category::where('category_id',$cid)->whereor('pid',$cid)->field('category_id')->select();
