	...
	<div id="detail_page">
		<div>
			<div class="details">
				<div class="details-l fl">
					...
					<div class="details-l-02">
						...
						<div class="details-l-02-r">{{details.users}}</div>
					</div>
					<div class="details-l-02">
					...
					<div class="details-l-02">
						...
						<div class="details-l-02-r">xxx</div>
					</div>
				</div>
				...
				<div class="details-r fl">
					<div class="details-r-01">
						<el-row :gutter="20">
							<el-col :span="6"><i class="el-icon-warning"></i> 订单状态：

								<span v-if="details.pay_status == 1">已支付
					...
					<el-row :gutter="20" style="font-size:14px;">
						<el-col :span="6" :offset="1">快递: {{details.courier}} {{details.courier_num}}</el-col>
						<el-col :span="6" :offset="8">商品总金额：¥{{details.order_money}}</el-col>
					</el-row>
			...
			<div class="log">
				...
				<div class="log-02">
					<ul>
						<li v-for="(item,index) of details.log">[{{item.create_time}}]
							<b>{{item.type_name}}：</b>{{item.content}}</li>
					</ul>
				</div>
			</div>
			<div class="clear"></div>
		...
		<el-dialog title="" width='400px' :visible.sync="form_shipping">
			<el-form :model="form" style=''>
				<el-form-item label="快递公司" label-width="80px">
					<el-input v-model="form.courier" auto-complete="off"></el-input>
				</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			post_details() {
				var that = this;
				this.http.post("/admin/get_order_one", {
					id: this.mid
				}).then((res) => {
					that.details = res.data; //订单详情 
				})
			},
			//删除商品
			del(id, index) {
			...
			courier_sub() {
				...
				this.http.post('/admin/up_order_courire', arr)
					.then((res) => {
